# graph/export/onnx_converter.py
...
def onnx_converter(state: TrainState) -> TrainState:
    """
    best.pt → best.onnx 변환만 수행. (평가는 하지 않음)
    결과 경로는 state.context['onnx_converter']와 state.registry_info에 기록.
    """
    print("[onnx_converter] ONNX 변환 시작")
    ...
    if onnx_path.exists():
        print(f"[onnx_converter] 기존 ONNX 발견 → {onnx_path}")
    else:
        ...
        print(f"[onnx_converter] export 완료 → {onnx_path}")
    ...
    reg["onnx_path"] = str(onnx_path)
    state.registry_info = reg

    print("[onnx_converter] 완료 ✅")
    return state
