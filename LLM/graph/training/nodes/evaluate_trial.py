...
_METRIC_ALIASES: Dict[str, List[str]] = {
    "mAP50-95": [
        ...
        "box/mAP50-95", "bbox_map50_95",
        "map_50_95", "seg/mAP50-95",
    ],
    "mAP50": [
        "mAP50", "map50", "metrics/mAP50",
        "box/mAP50", "bbox_map50",
        "seg/mAP50",
    ],
...
def _collect_metric_files(run_dir: Optional[Union[str, Path]]) -> List[Path]:
    ...
    globs = ["results*.json", "metrics*.json", "val*.json"]
...
def evaluate_trial(state: TrainState) -> TrainState:
    ...
    개선사항:
      - train_trial.save_dir, train_trial.run_dir 모두 자동 탐색
      - 파일이 없어도 state.metrics 기반 평가 수행
    ...
    if isinstance(state.context, dict):
        # evaluate_trial이 읽을 수 있는 여러 후보 키
        run_dir = (
    ...
    if files:
        ...
        if file_metrics:
            ...
            state.notes = f"[evaluate_trial] results 파일 {len(files)}개에서 메트릭을 읽었습니다."
    else:
        if not metrics:
